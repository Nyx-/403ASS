...
int main(int argc, char *argv[]) {
    ...
    /* Generate socket */
    //The function creates a socket with AF_INET ( IP Addressing ) and of type SOCK_STREAM. Data from all devices wishing to connect on this socket will be redirected to sock_fd
    if ((sock_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) { 
		perror("socket");
		exit(1);
	}
	...
	serv_addr.sin_port = htons(atoi(argv[1]));    /* short, network byte order (IP put in my server when you first run) */ 
